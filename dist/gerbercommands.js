...
class FSCommand {
    constructor(cmd, lineNo) {
        ...
        if (match[1]) {
            switch (match[1]) {
                case 'L':
                    coordZeros = primitives_1.CoordinateZeroFormat.LEADING;
                    break;
                case 'T':
                    coordZeros = primitives_1.CoordinateZeroFormat.TRAILING;
                    break;
                case 'D':
                    coordZeros = primitives_1.CoordinateZeroFormat.DIRECT;
                    break;
            }
        }
    ...
    formatOutput() {
        let result = "FS";
        if (this.coordinateFormat.coordFormat != primitives_1.CoordinateZeroFormat.NONE) {
            switch (this.coordinateFormat.coordFormat) {
                ...
                case primitives_1.CoordinateZeroFormat.TRAILING:
                    result += "T";
                    break;
                case primitives_1.CoordinateZeroFormat.DIRECT:
                    result += "D";
                    break;
            }
        }
...
function parseCoordinateY(coordinate, fmt) {
    return utils_1.parseCoordinate(coordinate, fmt.yNumIntPos, fmt.yNumDecPos, fmt.coordFormat);
}
function formatCoordinateX(value, fmt) {
    return utils_1.formatFixedNumber(value, fmt.xNumDecPos, fmt.xNumIntPos, fmt.coordFormat);
}
function formatCoordinateY(value, fmt) {
    return utils_1.formatFixedNumber(value, fmt.yNumDecPos, fmt.yNumIntPos, fmt.coordFormat);
}
class D01Command {
