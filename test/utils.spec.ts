...
describe("Utils tests", () => {
    it('Number format', () => {
        ...
        str = cm.formatFixedNumber(1, 3, 2, CoordinateZeroFormat.LEADING);
        assert.equal(str, "1000");
        str = cm.formatFixedNumber(-1, 3, 2, CoordinateZeroFormat.LEADING);
        assert.equal(str, "-1000");
        str = cm.formatFixedNumber(-1.23441, 3, 2, CoordinateZeroFormat.LEADING);
        assert.equal(str, "-1234");
        str = cm.formatFixedNumber(12.3441, 3, 2, CoordinateZeroFormat.LEADING);
        assert.equal(str, "12344");
        assert.throws(() => cm.formatFixedNumber(1234, 3, 2, CoordinateZeroFormat.LEADING));
        str = cm.formatFixedNumber(0.03, 3, 2, CoordinateZeroFormat.LEADING);
        assert.equal(str, "30");

        str = cm.formatFixedNumber(1, 2, 3, CoordinateZeroFormat.TRAILING);
        assert.equal(str, "001");
        str = cm.formatFixedNumber(-1, 2, 3, CoordinateZeroFormat.TRAILING);
        assert.equal(str, "-001");
        str = cm.formatFixedNumber(-1.23441, 2, 3, CoordinateZeroFormat.TRAILING);
        assert.equal(str, "-00123");
        str = cm.formatFixedNumber(-1.2, 2, 3, CoordinateZeroFormat.TRAILING);
        assert.equal(str, "-0012");
        str = cm.formatFixedNumber(123.441, 2, 3, CoordinateZeroFormat.TRAILING);
        assert.equal(str, "12344");
        assert.throws(() => cm.formatFixedNumber(1000, 2, 3, CoordinateZeroFormat.TRAILING));
    ...
    it('Number parse', () => {
        ...
        value = cm.parseCoordinate("0003", 2, 3, CoordinateZeroFormat.TRAILING);
        assert.ok(Math.abs(value - 0.03) < 1e-10);

        value = cm.parseCoordinate("30", 2, 3, CoordinateZeroFormat.LEADING);
        assert.ok(Math.abs(value - 0.03) < 1e-10);
        ...
        value = cm.parseCoordinate("500000", 2, 5, CoordinateZeroFormat.LEADING);
